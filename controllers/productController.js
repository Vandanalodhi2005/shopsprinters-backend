...
const createProduct = asyncHandler(async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            const imgs = req.body.images || req.body.existingImages;
            imageUrls = typeof imgs === 'string' ? JSON.parse(imgs) : imgs;
        }
...
const updateProduct = asyncHandler(async (req, res) => {
    ...
    if (product) {
        ...
        if (req.body.existingImages || req.body.images) {
            const imgs = req.body.existingImages || req.body.images;
            currentImages = typeof imgs === 'string' ? JSON.parse(imgs) : imgs;
        } else {
    ...
    for (let i = 0; i < jsonData.length; i++) {
        ...
        try {
            ...
            const productData = {
                ...
                images: row.images || row.Images || row.image || row.Image
                    ? (typeof (row.images || row.image) === 'string'
                        ? (row.images || row.image).split(',').map(s => s.trim())
