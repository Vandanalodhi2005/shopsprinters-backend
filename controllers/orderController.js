...
const addOrderItems = asyncHandler(async (req, res) => {
    ...
    if (orderItems && orderItems.length === 0) {
        ...
        io.emit('new-order', {
            id: createdOrder._id,
            message: `New order #ORD-${createdOrder._id.toString().substring(createdOrder._id.toString().length - 4).toUpperCase()} placed`,
            path: '/admin/orders',
            time: new Date()
        });
...
const createCloverPayment = asyncHandler(async (req, res) => {
  ...
  try {
      ...
      if (orderId) {
          try {
             // Order payment failed, keep status as default (Processing)
             const order = await Order.findById(orderId);
             if (order) {
                 await order.save();
             }
...
const getOrders = asyncHandler(async (req, res) => {
    ...
    if (fetchAll) {
        ...
        // Return in same format to avoid breaking frontend expectation of object if we switch
        // But since we are changing the contract, let's keep it clean.
        // Wait, current frontend expects ARRAY.
        // If I change to Object, I MUST update all consumers.
        // AdminOrders.jsx and AdminCustomers.jsx use this.
        res.json({ orders, page: 1, pages: 1, count: orders.length });
    } else {
